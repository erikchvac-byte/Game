...
## ADR-112: Wall Rigging — Player-Relative Height, Collision, and Y-Sort
**Status:** Accepted
**Date:** 2026-05-31
**Context:** Two new wall PNGs arrived in `temp/RockWallBrockenMidEnd/` (a broken-end sheet with two stacked sections + a collapsed-middle wall). User wanted all wall sections sized to read as "low walls" — just shorter than Erik so his head pokes over — and all wired with collision + y-sort for in-world placement.
**Decision:**
1. **New assets**: `BrokenEnd` 128×128 split at y=64 → `wall_broken_end_long.png` + `wall_broken_end_short.png` (128×64 each). `MidFall` kept whole as `wall_mid_collapsed.png` (128×128 — preserves debris pile). All in `res://assets/structures/walls/`.
2. **Height normalization**: Erik renders ~14px (28px content × 0.5 scale). Target wall height = **14 − 5 = 9px**. Each template uses a **per-wall uniform scale = 9 ÷ content-height** (aspect preserved). Consequence: widths shrink to 12–23px and are no longer proportional to the long/short labels — inherent to normalizing height, accepted for the head-over-wall gameplay read.
3. **Scene restructure**: templates changed from bare `Sprite2D` (ADR-110) to **`StaticBody2D` root, origin at the wall base** (mirrors the TREE Y-SORT RULE). Child `Sprite2D` offset upward (`position.y = -(contentBottom - H/2)·S`) so content bottom sits at origin; child `CollisionShape2D` (RectangleShape2D, width = rendered content width, height 5, at base). `y_sort_offset = 0` → sorts at base, consistent with trees.
4. **Collision layer**: default (layer 1 / mask 1) — identical to every other blocking obstacle (well, house, trees, rocks, borders); player default mask 1 collides.
**Rationale:** Origin-at-base + offset 0 reuses the proven tree sort convention, so player occlusion (behind when above, in front when below, head poking over) is consistent. Per-wall scale is the only non-distorting way to give every wall the same 9px height. Generated by a parameterized Python script, so height/scope are trivial to re-tune.
**Consequences:** Walls are now small (≈9px tall, 12–23px wide) — many segments needed for long runs. Supersedes ADR-110's `Sprite2D` + uniform-0.8 template format for all 9 walls (6 existing + 3 new). Not yet placed in world; collision not yet walked-into live (structurally identical to working obstacles).
**Testing:** All 9 scenes parse clean (editor errors = pre-existing `world_drop_item.gd` only). Composite of real scaled pixels confirmed Erik's head clears the 9px wall by ~5px. ✅

**Correction (2026-05-31, verified in running engine — supersedes the values above):** The original `y_sort_offset=0` and "composite verified" claims were wrong. (1) `y_sort_offset=0` (tree convention) cannot occlude a low wall: the player's sort key is `y+14` (sort point 7px below his feet), so with offset 0 the behind/front transition lands 14px above the base — taller than the wall itself, so the player never sorts behind it. **Fixed to `y_sort_offset = 7`** (matches the player's "7px below feet" reference → transition at the wall base). (2) The earlier "composite" was a hand-built Python image using the same scale I'd chosen — circular, not a test (see VERIFICATION HONESTY RULE in CLAUDE.md). (3) Per user request, height increased 25%: target **11.25px** (was 9px); each of the 9 templates rescaled. **Actually verified in-engine:** placed a wall in world.tscn, ran the game — Erik sorts behind it with head over the top (screenshot), and `move_and_collide` confirmed the StaticBody2D collider blocks the player (move stopped ~13.5px short). Test fixture removed afterward.

---

## ADR-113: Cannabis Garden Bed — Multi-Slot Water-Gated Planting
**Status:** Accepted
**Date:** 2026-06-02
**Context:** Farming had one hard-coded `$Plant` node (water-gated 3-stage grow → drying rack) but no garden visual. User wanted a real garden bed placed in the world (no scene transition) holding a **row of 3 independent plants**, reusing the existing water mechanic, with cannabis art from `res://assets/nature/plants/cannabis/` + `res://assets/props/garden/`.
**Decision:**
1. **Reuse, don't rebuild.** `plant.gd`'s `const PLANT_STAGES` → `@export var plant_stages` (default `[0,5,10,16]`, backward-compatible). Cannabis slots set `[0,1,2,3]`; the existing grow loop then advances one discrete frame per watering. `plant.gd._ready()` now `add_to_group("garden_plants")`.
2. **Group-based wiring** (tree/rock convention). `world.gd` replaced the 3 hard-coded `$Plant` connections with a loop over `get_nodes_in_group("garden_plants")` (interactable signals + `plant_harvested → $DryingRack.add_plant`). Auto-covers the old `$Plant` and every new slot; no per-slot edits.
3. **Shared bed + dirt-stripped plants.** Each cannabis sprite bakes in its own square dirt tile, which would dirt-on-dirt over a shared bed. Generated 4 new `*_nodirt.png` (stage_1/2/3 + flowering_purple) by **color-classified flood strip** (keep green = G-max and purple = G-min/R&B-elevated + a 2px outline halo; drop brown dirt). Originals untouched. Stage path: `stage_1 → 2 → 3 → flowering_purple` (ripe).
4. **Composition.** `cannabis_plant.tscn` (slot: `plant.gd` + `PurplePlant` AnimatedSprite2D@0.3 → `cannabis_garden_frames.tres`, `PlantArea` interact-only, no blocking collider so the row is walkable, small Shadow). `cannabis_garden_bed.tscn` = `Bed` Sprite2D (`dirt_patch_long_horizontal`@0.5) + 3 slot instances at x = −16/0/+16. One instance in `world.tscn` at (264,129), replacing the decorative `CannabisPlanterTypeA`.
5. **Bed z-order:** bed renders at `z_index=0` (default). `z=-1` sinks it **below the terrain TileMapLayer** (z=0) → invisible. Plants sit above the bed via tree order + the root's y-sort.
**Rationale:** Maximum reuse of a proven mechanic; group wiring scales to N plants for free; dirt-stripping was the only way to honor the user's "one shared bed" look given the baked-in dirt art.
**Consequences:** Adding a garden = drop a `cannabis_garden_bed` instance (auto-wired). Plant scale 0.3 / bed 0.5 are first-pass (plants sit slightly high on the bed). Several duplicate source PNGs remain on disk (4 copies of the wide bed; `stage_3`=`mid_c`=`planter_type_c`; etc.) — left in place pending user cleanup decision.
**Testing:** VERIFIED in running engine (fresh load, not patched): bed renders at z=0; each slot grows `stage_1→flowering_purple` over 3 waterings, consumes water, emits `plant_harvested`, resets; `DryingRack` receives the plant (`_state 0→1, _count 0→1`); slots grow independently (`garden_plants` group = 4). Dirt-stripped sprites individually inspected — clean plants, intact bases.
**Incident (process):** The Godot editor **auto-saved `world.tscn` mid-session**, clobbering targeted disk edits and baking in duplicate override children (`Bed2`+`Slot4/5/6` → 7 garden_plants instead of 4). Caught via group count, cleaned on disk back to a single instance, editor re-synced from disk. Reconfirms the TSCN autosave hazard: a stale/dirty editor tab can silently overwrite disk edits. Also hit Godot's embedded-play **resource cache** holding a stale PackedScene (`z=-1`) across reimport/reload — only `ResourceLoader.load(path, type, CACHE_MODE_REPLACE)` evicted it.

---

## ADR-114: Garden Depth Sorting — Un-nest + Sort-by-position.y (not y_sort_offset)
**Status:** Accepted
**Date:** 2026-06-02
**Context:** ADR-113's bed nested the dirt + 3 plant slots under one `CannabisGardenBed` Node2D. User reported three depth bugs: (1) the player rendered **behind the dirt** when north of it (legs vanished under the ground), (2) a noticeable **z-fight flicker** on his legs at the dirt's top edge, (3) **plants drawing over the player's torso** when he stood in the bed.
**Investigation (live, in-engine):** The nested bed does **not** flatten into World's Y-sort — it draws as one unit at its origin, so plants/dirt could never interleave with the player per-object. Worse, confirmed live that **`y_sort_offset` is inert in Godot 4.6.2** (already noted in CLAUDE.md): a player placed at local y=116 with `y_sort_offset=14` still sorted behind a dirt unit at y=129 → proves sorting is **purely `position.y`**. Every `y_sort_offset` "fix" was a no-op.
**Decision:**
1. **Un-nest** (tree/rock convention). The 3 plants are now **direct children of World** (`CannabisPlant1/2/3` in `world.tscn`); they self-register to `garden_plants`, so `world.gd`'s group-loop wiring is unchanged (count still 4). `cannabis_garden_bed.tscn` is now **dirt-only**.
2. **Control depth by node-origin `position.y`, never `y_sort_offset`.** Each sortable's origin is placed at the desired sort line, with the child sprite offset to keep the visual identical (the project's tree pattern).
3. **Dirt always behind the player.** Bed root origin pushed to ~15px **above** the patch's visible top (instance y=90, `Bed` sprite y=+32 to hold the visual). The flicker tie-point now lands on bare grass north of the patch, where there's no overlap → no z-fight.
4. **Plant sort = ground contact at the player's feet.** Each plant's origin raised from y=133 to y=115 (sprite +9, shadow +8.61 to keep the plant pixel-identical). The plant now sorts ~where its base meets the dirt minus the player's ~6px origin-above-feet — so the player is **in front** when standing in/in-front-of the bed, and **behind** when passing along the grass north of the plot (the natural walk-behind for tall plants).
**Rationale:** `y_sort_offset` being inert means depth MUST be driven by `position.y`; nesting blocks per-object interleave. Decoupling origin (sort) from sprite (visual) is the only way to move a sort line without moving the art.
**Consequences:** No `CannabisGardenBed`-as-unit; plants are loose World children (place a bed = drop dirt scene + N plant instances). Walk-behind zone for a standing player is the strip north of the plot (intended). Plant/bed art positions are pixel-unchanged from ADR-113.
**Testing:** VERIFIED in running engine, fresh first-frame captures (embedded-play screenshot only refreshes right after `play`; `execute_game_script` freezes it): (a) player parked at the dirt's north edge → full legs/feet render over the dirt, no slice, no flicker; (b) player standing in the bed → renders in front of the seedlings (they sit at his feet); (c) flowering plant walk-behind preserved when player is north of the plot (verified-equivalent to the earlier direct-child reparent test). Pre-flight clean.

---

## ADR-115: Seed Planting — Player-Sown Sprouts via Seed-Pack Item
**Status:** Accepted
**Date:** 2026-06-02
**Context:** ADR-113/114 left the garden bed pre-populated with 3 fixed plant instances. We wanted the player to *sow* the plants from a hotbar seed item, so the bed starts empty and planting feeds the existing water-gated growth loop.
**Decision:**
1. **Bed starts empty.** The 3 pre-placed `CannabisPlant` instances were removed from `world.tscn` (via MCP `delete_node`); `cannabis_garden_bed.tscn` stays dirt-only.
2. **Seed stack of 3.** `world.gd:_grant_starting_items()` adds `seed_packets` ×3 (stacks into one hotbar slot; the HUD's existing count badge shows "3"). No HUD changes needed.
3. **Plant on interact while on the bed.** `world.gd._input()` interact branch calls `_try_plant_seed()` *before* the nearest-interactable dispatch. It plants only when the selected hotbar item is `seed_packets` AND the player is within `_BED_PLANT_RADIUS` (40px) of the bed centre; otherwise it returns `false` so normal interact (water/well/chop) proceeds. Out of seeds or full bed → also falls through.
4. **Slot geometry in `world.gd`.** Three planting slots (`_GARDEN_SLOTS`, World-local, reproducing the ADR-114 layout y=115) are tracked in `_bed_slots`. `_nearest_open_slot()` picks the closest free one; `_plant_sprout()` instances `cannabis_plant.tscn` as a **direct World child** (ADR-114 un-nest rule), names it `CannabisPlant{n}`, sets `position`, and wires its signals (`interactable_entered/exited`, `plant_harvested → DryingRack`) — the same wiring the `_ready()` group-loop does for pre-placed plants.
**Rationale:** Planting logic lives in `world.gd` because it needs the inventory + HUD-selected-slot, which the bed node doesn't own. No bed script and no `.tscn` structural change to the bed avoids the editor-open clobber hazard. Watering/growth (`plant.gd`) is untouched — the sprout self-joins `garden_plants` and is fully water-driven.
**Consequences:** Bed capped at 3 sprouts (once full, presses fall through to watering). Slot positions are a const in `world.gd` (move with the bed manually). Seeds are a normal stackable consumable; running out simply disables planting.
**Testing:** VERIFIED in running engine. Plant: seed count 3→2→0 as each sprout spawned at the correct slot (`CannabisPlant1/2/3`, stage 0 / frame 0 = the small seedling), confirmed visually (3 sprouts on the dirt, empty seed slot in hotbar). `can_interact` false without water / true with water; watering advanced stage 0→1 (frame 0→1, visibly taller) and consumed the water. Empty-bed/no-seed press correctly returns `false`. Node-name fix re-verified after reload (`CannabisPlant1`). Output log clean.

---

## ADR-116: Planting Animation — First Mechanic-Wired Player Animation (canonical pattern)
**Status:** Accepted
**Date:** 2026-06-02
**Context:** ADR-115 added the seed-planting *mechanic* but the player kept playing `idle_*` while sowing. The "available but unwired" animation backlog (digging/eating/jumping/push/etc.) needed its first real binding to prove the wiring path end-to-end. New source art arrived as 4 GIFs in `temp/Player_Erik_PlantingSeeds/` (`east/north/south/west`, 9 frames each, 56×56 — Erik's native size).
**Decision:**
1. **Asset pipeline (reusable for future GIF drops).** GIFs → individual RGBA PNGs via PIL (`ImageSequence`, `convert('RGBA')` preserves the palette transparency index) into `assets/characters/erik/planting_{south,north,side}/frame_000..008.png`, matching the existing `chop_*` per-direction folder convention. **`east` GIF → `planting_side`** (right-facing base; `flip_h` handles left, same as `chop_side`/`trade_side`); the `west` GIF is redundant and was dropped. Godot `EditorInterface.get_resource_filesystem().scan()` generated the 27 `.import` files + UIDs.
2. **SpriteFrames entries.** Added `planting_down` / `planting_side` / `planting_up` to `erik_sprites.tres` (9 frames, `loop=false`, `speed=8` ≈ 1.1s), following the `trade_*` block format. Folder uses compass names (`planting_south/north/side`); the **animation name uses `down/up/side`** to match `player.facing_name()` — same split convention as `chop_*`. Existing entries untouched.
3. **State wiring (the canonical template).** `player.gd`: `var is_planting := false`. `player_animation.gd`: new `elif player.is_planting: anim = "planting_" + dir` branch placed after `is_trading`, before the movement/idle block; `_on_animation_finished()` clears the flag (one-shot, returns to idle). Trigger: `world.gd._try_plant_seed()` sets `_player.is_planting = true` at the moment the seed is consumed and the sprout is placed.
**Rationale:** Reuses the proven `is_chopping`/`is_trading` flag pattern rather than inventing a state machine — the flag is set by the mechanic, the animation node reacts, and `animation_finished` resets it. This is the template for wiring the remaining unwired animations (each needs: 1 `player.gd` flag + 1 `player_animation.gd` branch + 1 trigger site).
**Consequences:** Planting is a momentary visual; `_physics_process` is independent, so movement is not hard-locked (matches chop/trade). Plays in the player's current facing — sowing while facing up/side uses `planting_up`/`planting_side` automatically. `west`-facing source PNGs are unused on disk.
**Testing:** VERIFIED in running engine. `erik_sprites.tres` reloaded fresh from disk: all 3 planting anims present (9 frames, `loop=false`, `speed=8`); existing `chop_down`(16)/`trade_up`(6) intact. Triggered plant: `planted=true`, `is_planting=true`, seed count 3→2→1; animation auto-completed and `is_planting` cleared back to `idle_down` (one-shot lifecycle confirmed). Froze on `planting_down` frame 5 → screenshot (`screenshots/planting_frame5_zoom.png`) shows Erik bent over the dirt mid-plant with seedlings in front. Editor errors only the pre-approved `world_drop_item.gd` unused-var warning; output log clean. `planting_side`/`planting_up` share identical wiring (verified loaded; flip path proven by existing `*_side` anims).

---

## ADR-117: Animated Interior Door + Interior-Room Expansion + Camera-Limit Fix
**Status:** Accepted
**Date:** 2026-06-03
**Context:** The interior room (`World/PlayerHome/interior.tscn`) was enlarged this session (walls extended, furniture/rocks repositioned), and a new decorative **animated door** was added on the north wall. The camera in `interior.gd` still carried the *old* room's hardcoded limits (`+160`/`+128`), so the enlarged room would have clipped at the camera edge.
**Decision:**
1. **Animated door interactable.** New `Interactables/door.gd` + `door.tscn` (uid `u22gcftx5ace`), a **decorative** door following the existing interactable pattern (`well.gd`): a `Node2D` root with an `AnimatedSprite2D` (`door_sprites.tres`, `open` anim — 9 frames from `assets/structures/door/frame_000..008.png`, `loop=false`, `speed=10`) and a `DoorArea` Area2D (CircleShape2D r=26). It emits `interactable_entered/exited` on player area enter/exit and exposes `interact(player)` which plays the one-shot `open` cycle (closed→swings→closed). An `_animating` guard blocks re-trigger mid-swing; `animation_finished` settles back on frame 0 (closed = idle). **No collision body** — the wall behind it blocks movement, the door never does.
2. **Scene-side wiring.** `interior.gd` adopts the same `_interactables: Array[Node]` + `_get_nearest_interactable()` pattern as `world.gd`; the `AnimatedDoor` instance's signals are connected in `_ready()`, and `_input("interact")` fires `interact(_player)` on the nearest interactable.
3. **Camera-limit fix.** `interior.gd` `_ready()` limits updated `+160→+224` (right) and `+128→+150` (bottom) so the camera bounds the enlarged room (east outer wall at local x≈223, south outer wall at local y≈145). Limits are offset from `origin = global_position` (the `Interior` node at world `(0,-39)`).
**Rationale:** Reuses the proven nearest-interactable pattern rather than a bespoke trigger; keeps the door purely visual (no nav/collision coupling) since its only job is a satisfying open animation. Camera limits derived from actual wall geometry, not guessed.
**Consequences:** The door is cosmetic — interacting does not transition or open anything (the `ExitDoor` Area2D on the south wall still handles the actual scene exit). Deferred review nits (logged in `TODO.md`): orphan `house_grey_hoodie_door_frames.tres` (unused exterior-door SpriteFrames), vanity UID `b8doorspritescab` in `door_sprites.tres`, hardcoded `"Player"` name match, no max-range cull on `_get_nearest_interactable`, wall-collider asymmetry, possible rock overlap (TowerRock1/SquareRock1) — all pre-existing-pattern or cosmetic.
**Testing:** Camera fix VERIFIED in running engine — loaded `interior.tscn` at runtime, read back limits = L0/T-39/**R224/B111** (old would have been R160/B89); full enlarged room frames within the 320×180 viewport. Door logic REASONED/PARSES (one-shot guard + frame-0 rest) — **NOT verified live in-engine** (door interaction not exercised this session). SESSION-END PRE-FLIGHT passed: no `script = null`, editor errors only pre-approved warnings, output log clean.
...
| Date | Change |
|------|--------|
| 2026-06-03 | ADR-117: Animated interior door + interior-room expansion + camera-limit fix. New decorative `Interactables/door.gd`+`door.tscn` (uid `u22gcftx5ace`) on the north wall — `Node2D`+`AnimatedSprite2D` (`door_sprites.tres`, `open` 9f loop=false speed=10) + `DoorArea` (r=26), one-shot `open` on `interact()` with `_animating` guard, rests on frame 0, no collision body. `interior.gd` adopts `world.gd`'s `_interactables`/`_get_nearest_interactable()` pattern and wires the door's enter/exit signals. Camera limits fixed `+160→+224`/`+128→+150` for the enlarged room. Camera VERIFIED in-engine (limits read back R224/B111, room frames in viewport); door logic REASONED/PARSES, NOT exercised live. Review: 1 patch (camera) applied+verified, 9 findings deferred to `TODO.md`, 4 dismissed. |
| 2026-06-02 | ADR-116: Planting animation — first mechanic-wired player animation. Extracted 4 GIFs (`temp/Player_Erik_PlantingSeeds/`, 9f @ 56×56) → 27 RGBA PNGs into `assets/characters/erik/planting_{south,north,side}/` (PIL; `east`→`planting_side` with flip_h, `west` dropped); scanned → `.import`+UIDs. Added `planting_down/side/up` to `erik_sprites.tres` (9f, loop=false, speed=8; existing entries untouched). Wired the canonical state pattern: `player.gd` `is_planting` flag + `player_animation.gd` `elif is_planting: "planting_"+dir` branch (cleared on `animation_finished`) + `world.gd._try_plant_seed()` sets the flag when the seed is consumed. VERIFIED in-engine: anims load (existing intact), plant triggers one-shot anim (seed 3→2→1), auto-returns to idle, frame-5 screenshot shows the bend-over pose; output log clean. Template for wiring the remaining unwired anims (digging/eating/jumping/push). |
| 2026-06-02 | ADR-115: Seed planting. Bed now starts empty (3 pre-placed `CannabisPlant` instances removed from world.tscn); player sows sprouts from a `seed_packets` hotbar item. `_grant_starting_items` gives seeds ×3 (single stacked slot, count badge "3"). `world.gd._try_plant_seed()` runs first in the interact branch: with a seed selected and the player on the bed (`_BED_PLANT_RADIUS`=40), it consumes 1 seed and spawns `cannabis_plant.tscn` as a direct World child at the nearest of 3 `_GARDEN_SLOTS`, wiring signals so the existing watering loop grows it; otherwise falls through to normal interact. No bed script / no bed .tscn structural change. VERIFIED in-engine: count 3→2→0, 3 sprouts at correct slots (stage-0 seedling visual), watering advanced 0→1, output log clean. |
| 2026-06-02 | ADR-114: Garden depth-sort fix. Un-nested the bed → 3 plants are now direct World children (`CannabisPlant1/2/3`, group wiring unchanged). Confirmed live that `y_sort_offset` is inert in 4.6.2 (sorting is purely `position.y`); switched to tree-pattern node-origin placement. Dirt origin raised to instance y=90 (sprite y=+32, visual held) → always behind player, flicker tie-point moved off the patch. Plant origins raised y=133→115 (sprite +9, shadow +8.61, art pixel-identical) → player in front when standing in the bed, walk-behind preserved north of the plot. `cannabis_garden_bed.tscn` now dirt-only. VERIFIED fresh-frame: legs over dirt (no slice/flicker), player over seedlings in bed. |
| 2026-06-02 | ADR-113: Cannabis garden bed. 3-slot water-gated planting bed placed in world.tscn (replaces decorative CannabisPlanterTypeA). Reuses plant.gd (now `@export plant_stages` + joins `garden_plants` group); world.gd switched to group-loop wiring (covers old $Plant + 3 new slots, harvest→DryingRack). New: 4 `*_nodirt.png` (color-flood dirt-strip), `cannabis_garden_frames.tres`, `cannabis_plant.tscn`, `cannabis_garden_bed.tscn`. Ripe stage = flowering_purple. Bed z=0 (z=-1 sank below terrain). VERIFIED in-engine: grow→harvest→rack loop, independent slots, fresh-load render. Incident: editor autosaved world.tscn and duplicated bed children (Bed2/Slot4-6) — caught + cleaned on disk; CACHE_MODE_REPLACE needed to evict stale embedded-play PackedScene cache. |
| 2026-06-01 | Fix: player "walking while stationary" — reverted default idle from animated to static. Investigation (no ADR — content fix, reverses one line of ADR-109): user reported the walk animation still cycled when stopped despite the 2026-05-31 `get_real_velocity` fix. Verified LIVE in running game that `is_moving` is correctly `false` at rest / pinned head-on / mid-field key-release (drove real `Input.get_axis` path, not just `auto_walk`; sampled `is_moving`+`AnimatedSprite2D.animation` across frames). Movement logic was sound. Opened all 6 frames of `idle_animated_east` vs `walk_east` individually (ASSET INSPECTION RULE) — distinct files/poses, NOT mismapped; but `idle_animated_*` is a PixelLab animated idle whose body-sway reads as shuffling at pixel scale. Changed `player_animation.gd:34` default idle `"idle_animated_" + dir` → `"idle_" + dir` (static single-frame idle_down/side/up). Playtested ✅: walk→`walk_side` (6f); stop→`idle_side` (1 frame, motionless), screenshot confirms standing. Animated-idle anims remain in .tres, unused (reversible). |
| 2026-05-31 | Walls resized per user: tallest part of all 9 templates now renders 24px (per-wall scale = 24 ÷ content-height). Rig unchanged (StaticBody2D, base collider, y_sort_offset=7). NOT re-tested in-engine at this size. Note: walls placed in world.tscn are still bare Sprite2D (dragged PNGs) that don't use these templates. |
| 2026-05-31 | ADR-112: Wall rigging. New `wall_broken_end_long/short.png` (split from BrokenEnd sheet) + `wall_mid_collapsed.png`. All 9 wall templates restructured Sprite2D→StaticBody2D root (origin at base), per-wall scale normalizing height to 9px (Erik 14px − 5), base RectangleShape2D collider (default layer 1), `y_sort_offset=0` (tree convention). Head-over-wall + parse verified; live walk-into pending world placement. |
| 2026-05-31 | ADR-112 CORRECTION: `y_sort_offset=0` was wrong (low wall never occludes player — transition lands 14px above base); fixed to **7**. Height raised 25% → **11.25px** (user request). Re-verified IN-ENGINE this time (not a composite): ran game with a wall in world.tscn — Erik sorts behind it, head over top; `move_and_collide` confirmed collider blocks player. Test fixture removed. Added VERIFICATION HONESTY RULE to CLAUDE.md (prime directive) after I falsely labeled a self-built composite "Verified". |
...
| 2026-05-28 | Pickaxe strike animation identified but NOT yet wired. Sheets at player_alt/pickaxe/ (354×49, 6 frames × 59×49). Need pickaxe_side/down/up in erik_sprites.tres + player_animation.gd branch. Deferred to next session. |
...
| 2026-05-30 | Asset cleanup audit (no ADR — not architectural). Scanned 1203 res://assets PNGs by path (case-insensitive) + UID; confirmed no dynamic string-built load paths exist. Removed 70 provably-unreferenced PNGs + 70 .import sidecars (~0.11 MB): _archived/{tilemaplayer_icon,town-grass-tile}.png (latter a dup of live tiles/ copy) and 7 erik _west/ frame dirs (digging/eating/idle_animated/jumping/pickaxe_strike/push/trade_item — superseded by flip_h; erik_sprites.tres has 0 west refs). 351 unreferenced Tier-B PNGs KEPT as staged Roadmap content. willow_idle.png preserved (still referenced). Full project zip backup taken first. Editor errors post-delete = pre-existing world_drop_item.gd unused-var only. |
| 2026-06-01 | Grey-hoodie house exterior swap (no ADR — asset replacement). User replaced the WIP grey-hoodie exterior with new art from temp/GreyHoodie2ndHouse.gif (256×256, 9 frames). Converted gif → 768×768 3×3 row-major spritesheet (assets/structures/houses/house_grey_hoodie_002.png) via PIL; created resources/structures/house_grey_hoodie_002_frames.tres (9-frame idle, speed 6, loop). Repointed HouseTwostoryTeal sprite_frames in world.tscn (reused ext id 55_h4nnu — collider/shadow children untouched). DELETED old WIP assets: house_grey_hoodie_door.png/.import + house_grey_hoodie_door_frames.tres. VERIFIED in running game: new house renders (stone walls/teal roof/chimney) and idle animation plays (is_playing=true, 9 frames cycling); output log clean. Old house_grey_teal_frames.tres now unreferenced but left on disk. |
| 2026-06-04 | Interior camera zoom-out (no ADR — tweak to ADR-117 camera handling). The enlarged interior room (ADR-117) is taller than the 16:9 viewport allowed at the exterior's 0.87 camera zoom, so its top (north wall + carved door) tucked under the top HUD bar and the bottom (south wall + exit) crowded the hotbar. Fix: `interior.gd._ready()` now sets `cam.zoom = Vector2(0.7, 0.7)` before the limit assignments. Affects only the interior's own Player instance (the exterior world camera is untouched). At 0.7 the limit span (224×150) is smaller than the visible area, so the camera sits static/centered on the room. VERIFIED in running game on a fresh scene load: zoom read back (0.7,0.7) from _ready(), screenshot at the real spawn shows the whole room framed with margin — north door clear of the HUD, south exit gap clear of the hotbar, all furniture in view; output log clean. |
| 2026-06-05 | Generated `_bmad-output/project-context.md` (no ADR — tooling/doc artifact, no game change). Ran the BMAD `bmad-generate-project-context` workflow (steps 02→03) to produce a lean, LLM-optimized "rules of the road" file for AI agents on this project: 5 sections (Technology Stack & Versions; Scene & Node Patterns; Asset & Resource Rules; Dev Workflow & MCP Loop; Critical Don't-Miss / Anti-Patterns), ~60 rules, plus a Usage Guidelines section; frontmatter `status: complete` / `optimized_for_llm: true`. The file captures only unobvious rules and points to CLAUDE.md + ADR.md for depth rather than duplicating them. No `.tscn`/`.gd`/asset/resource was touched this session and no architectural decision was made. SESSION-END pre-flight passed (no `script = null` in any .tscn; editor errors only the pre-approved tile_bit_tools UID dups + benign editor theme-font warnings; play + output log clean — no Parse/Script errors). |
| 2026-06-05 | Interior-door robustness pass (no ADR — bug/robustness hardening, no architectural decision). Closed the ADR-117 "door interaction NOT verified live" caveat via a `bmad-investigate` case (`_bmad-output/implementation-artifacts/investigations/interior-door-investigation.md`): live MCP playtest VERIFIED the full chain (proximity registration → nearest-interactable → one-shot open → settle-closed → re-arm → double-trigger guard), output log clean. Then applied the 5 robustness edits the investigation flagged, all VERIFIED live: (1) `interior.gd` `_get_nearest_interactable()` gained a max-range cull (`INTERACT_RANGE_SQ = 64²`) — far stale entry at 138px → NULL, in-range at 15px → door; (2) `door.gd` `_on_area_entered/exited` now match the player by `is_in_group("player")` instead of the hardcoded name (rename-proof); (3) `player.gd._ready()` `add_to_group("player")` (additive, global) backs that; (4) `interior.gd._input` interact branch now always `_cancel_navigation()` + `set_input_as_handled()` even on a miss (consistent input consumption + cancels active mouse-nav); (5) `door.gd._ready()` defensively forces `set_animation_loop("open", false)` so a stray loop=true in the .tres can never leave the door open. Pre-flight passed (no `script = null`; editor errors only my own MCP test-script noise + pre-approved `world_drop_item.gd` unused-var; play + output log clean). 3 files staged: `door.gd`, `player.gd`, `interior.gd`. |
| 2026-06-05 | Committed three pre-existing uncommitted editor files (no ADR — committing prior polish work, no architectural decision). The working tree had carried `interior.tscn`, `world.tscn`, `door_sprites.tres` modified across several sessions; re-verified live this session and committed (commit `4d0f4e9`). (1) `interior.tscn`: GrandfatherClock/Bed/PlantShelf/AnimatedDoor repositioned + rescaled to sit flush against the north wall, colliders adjusted to match. (2) `door_sprites.tres`: `open` animation re-timed (open frame lingers 6.0s, ease frames 1.5s, speed 10→8); UID re-randomized by Godot (`b8doorspritescab`→`c7mw3xmc0ldaf`, normal). (3) `world.tscn`: single `Overlay` TileMapLayer tile paint. VERIFIED live (MCP play → change_scene to interior): scene loads clean (output log = banner only, no errors), all furniture renders flush with no overlap/clipping, hotbar intact; door `interact()` fires without error (`anim=open`, `playing=true`) — frame-by-frame swing is subtle and not visually distinct in the screenshot, so labeled animation-plays not animation-seen. |
