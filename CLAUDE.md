...
## Where We Are
- **Current state (2026-06-03):** Runnable, pre-flight ✅, output log clean (1 known unused-var warning in world_drop_item.gd — not blocking). All systems ADR-105–117 live. Full per-feature history lives in `ADR.md` — this section stays lean (newest decisions only).
...
### Session end — 2026-06-03 (interior door + room expansion, ADR-117)
- **Game state:** Runnable. Pre-flight ✅ (no `script = null` in any `.tscn`; editor errors only the pre-approved `world_drop_item.gd` unused-var warning; `play` + output log clean — no Parse/SCRIPT errors).
- **What changed this session:** (1) Adversarial code review (`/bmad-code-review`) of the uncommitted working tree (animated interior door + enlarged interior room). Triaged 2 reviewers → 1 patch, 9 defer, 4 dismiss. (2) **Applied + VERIFIED the camera patch:** `interior.gd` limits `+160→+224`/`+128→+150` to bound the enlarged room (east wall local x≈223, south wall local y≈145); read back live as L0/T-39/R224/B111, full room frames in the 320×180 viewport. (3) New **decorative animated door** (ADR-117): `Interactables/door.gd`+`door.tscn` (uid `u22gcftx5ace`), `Node2D`+`AnimatedSprite2D` (`door_sprites.tres`, `open` 9f loop=false speed=10) + `DoorArea` (r=26). `well.gd` interactable pattern; one-shot `open` on `interact()` with `_animating` guard, rests on frame 0; **no collision body** (cosmetic — wall blocks, `ExitDoor` on the south wall does the real scene exit). `interior.gd` adopts `world.gd`'s `_interactables`/`_get_nearest_interactable()`. (4) Saved the 9 deferred review findings to `TODO.md`.
- **⚠ Door interaction NOT verified live** — only the camera fix was exercised in-engine. The door's open animation + interact wiring **parses/reasons** but was **not played in the running game** this session (PLAYTEST RULE outstanding for the door specifically).
- **Tooling notes:** `play_scene` always plays "main" mode — to load a non-main scene at runtime, use `execute_game_script` + `change_scene_to_file` (change_scene is deferred → split into 2 calls: load, then read state). `execute_game_script` indentation **must be tabs** (spaces → "Mixed use of tabs and spaces" parse error), no `await`, use `_mcp_print()`.
- **⚠ PENDING — push likely still blocked:** prior session noted local `main` tracks `origin/master` and the safety classifier denied a direct default-branch push (ADR-115/116/117 commits may still be unpushed). This session's doc commit will be local too. To push: user runs `! git push origin HEAD:master`, OR feature branch + PR, OR add a Bash permission rule.
- **Open issues:**
  - **Door not playtested** — see above; exercise it in-engine next session (enter range → Space → confirm one-shot open + rest closed).
  - **9 deferred review findings in `TODO.md`** (section "Code Review — Deferred Findings"): orphan `house_grey_hoodie_door_frames.tres` (delete needs approval), possible TowerRock1/SquareRock1 overlap (needs visual playtest), vanity UID `b8doorspritescab`, no max-range cull on `_get_nearest_interactable`, hardcoded `"Player"` name match, wall-collider asymmetry, input-consumption inconsistency, door rest-frame loop dependency, no facing/nav-cancel gate.
