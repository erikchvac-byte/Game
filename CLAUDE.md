...
### Session end — 2026-06-05 (project-context.md generation)
- **Game state:** Runnable. Pre-flight ✅ (no `script = null` in any `.tscn`; editor errors only the pre-approved `world_drop_item.gd` unused-var + tile_bit_tools UID dups + benign editor theme-font warnings; `play` + output log clean — no Parse/SCRIPT errors). **No game code/scene/asset changed this session.**
- **What changed this session:** Generated **`_bmad-output/project-context.md`** via the BMAD `bmad-generate-project-context` workflow (steps 02→03) — a lean, LLM-optimized "rules of the road" file for AI agents: 5 sections (Technology Stack & Versions; Scene & Node Patterns; Asset & Resource Rules; Dev Workflow & MCP Loop; Critical Don't-Miss / Anti-Patterns), ~60 rules, a Usage Guidelines section, frontmatter `status: complete` / `optimized_for_llm: true`. It captures only unobvious rules and **defers to CLAUDE.md + ADR.md for depth**. No architectural decision made (change-log row only, no numbered ADR).
- **⚠ Pre-existing uncommitted game files (NOT mine):** at session start the working tree already had `M game/World/PlayerHome/interior.tscn`, `M game/World/world.tscn`, `M game/resources/structures/door_sprites.tres` (unverified, from a prior session). I left them untouched and **excluded them from this session's doc-only commit.** Decide next session whether to commit, revert, or re-verify them.
- **Tooling notes:** `play_scene` always plays "main" mode — to load a non-main scene at runtime, use `execute_game_script` + `change_scene_to_file` (change_scene is deferred → split into 2 calls: load, then read state). `execute_game_script` indentation **must be tabs** (spaces → "Mixed use of tabs and spaces" parse error), no `await`, use `_mcp_print()`. The MCP bridge needs a live editor link — if "Godot not connected", toggle the MCP Pro plugin off/on (or reload the project) to re-establish it.
- **⚠ PENDING — push likely still blocked:** local `main` tracks `origin/master` and the safety classifier has denied direct default-branch pushes before (ADR-115/116/117 + this session's doc commit may all be unpushed). To push: user runs `! git push origin HEAD:master`, OR feature branch + PR, OR add a Bash permission rule.
- **Open issues:**
