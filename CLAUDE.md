...
- **⚠ Pre-existing uncommitted game files (NOT mine):** the working tree still has `M game/World/PlayerHome/interior.tscn`, `M game/World/world.tscn`, `M game/resources/structures/door_sprites.tres` from a prior session — left untouched again and **excluded from this session's commit.** Note: `door_sprites.tres` was read this session and its UID is normal random `uid://c7mw3xmc0ldaf` (NOT the old vanity `b8doorspritescab` — that TODO finding is **resolved**). Decide next session whether to commit/revert/re-verify these three.
