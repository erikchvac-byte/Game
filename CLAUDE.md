...
## Where We Are
- **Last completed:** Choppable tree scenes (ADR-045). Four world trees converted to reusable `choppable_tree.tscn` instances — each has independent chop counter (3 hits), tree→stump state, and emits `wood_chopped` signal on completion. Previously: axe tool + wood resource (ADR-044).
