...
## Where We Are
- **Last completed:** Axe tool + wood resource integration (ADR-044). `player.equipped_tool` var added; C key toggles axe equip/unequip via world.gd; axe + bud + wood granted at start via `_grant_starting_items()`; all three appear in hotbar slots 1–3. Previously: InventoryManager autoload refactor (ADR-043).
...
- **Axe/tool pattern:** `player.equipped_tool: String` (empty = nothing equipped). C key → `_handle_axe_toggle()` in world.gd checks `InventoryManager.has_item("axe")` then flips `player.equipped_tool`. No axe animations yet — equip state is data-only for now.
- **Wood placeholder:** `res://GameAssets/Caves/Rocks/rock3.png` used as wood icon. Key = `"wood"`. No chop mechanic yet — granted at start for testing.
- **Next up:** Teal house collision refinement (door gap + side walls); cave entrance rigging; roof overlay for teal house in Overhead; dedup `shop_apothecary_alt.png`; dedicated gem sprite to replace WaterGem.png placeholder; tree-chopping mechanic with axe. Town/ and Village/ folders (~250 files each) still use numbered/mixed-case names — next naming pass.
