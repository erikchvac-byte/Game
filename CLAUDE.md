...
## Where We Are
- **Current state (2026-06-04):** Runnable, pre-flight ✅, output log clean (1 known unused-var warning in world_drop_item.gd — not blocking). All systems ADR-105–117 live. Full per-feature history lives in `ADR.md` — this section stays lean (newest decisions only).
...
### Session end — 2026-06-04 (interior camera zoom-out)
...
- **What changed this session:** Single fix — **interior camera zoom-out** (no ADR; tweak to ADR-117 camera handling). The enlarged interior room was taller than the 16:9 viewport allowed at the exterior's 0.87 zoom, so the north door tucked under the top HUD bar and the south exit crowded the hotbar. `interior.gd._ready()` now sets `cam.zoom = Vector2(0.7, 0.7)` immediately before the limit assignments. Affects only the interior's own Player instance (exterior camera untouched). At 0.7 the limit span (224×150) < the visible area, so the camera sits static/centered on the room. **VERIFIED in-engine on a fresh scene load** (not a runtime hack): zoom read back (0.7,0.7) from `_ready()`; screenshot at the real spawn shows the whole room framed with margin — north door clear of the HUD, south exit gap clear of the hotbar, all furniture in view; output log clean.
...
- **⚠ PENDING — push likely still blocked:** local `main` tracks `origin/master` and the safety classifier has denied direct default-branch pushes before (ADR-115/116/117 + this session's commits may all be unpushed). To push: user runs `! git push origin HEAD:master`, OR feature branch + PR, OR add a Bash permission rule.
- **Open issues:**
