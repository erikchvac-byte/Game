...
## Where We Are
- **Last completed:** Choppable tree scenes (ADR-045) — play-tested 2026-05-15, confirmed working. Four world trees converted to reusable `choppable_tree.tscn` instances — each has independent chop counter (3 hits), tree→stump state, emits `wood_chopped` signal, grants 1 wood to inventory on completion. Axe-equip guard verified. Previously: axe tool + wood resource (ADR-044).
