...
## Where We Are
- **Current state (2026-06-02):** Runnable, pre-flight ✅, output log clean (1 known unused-var warning in world_drop_item.gd — not blocking). All prior systems working (ADR-105–113 all live). **Cannabis garden bed (ADR-113, 2026-06-02):** 3-slot water-gated planting bed in `world.tscn` at (264,129), replacing the decorative `CannabisPlanterTypeA`. Reuses `plant.gd` (now `@export plant_stages`, default `[0,5,10,16]`; cannabis slots use `[0,1,2,3]`) + joins group `"garden_plants"`; `world.gd` switched to a group-loop that wires the old `$Plant` AND the 3 new slots (interactable signals + `plant_harvested → $DryingRack.add_plant`). Plants are **dirt-stripped** (`assets/nature/plants/cannabis/nodirt/{stage_1,stage_2,stage_3,flowering_purple}_nodirt.png`, color-flood strip; originals untouched) → `resources/plants/cannabis_garden_frames.tres` (4 frames). Scenes: `scenes/interactables/garden/cannabis_plant.tscn` (slot, plant@0.3, interact-only no blocking collider) + `scenes/structures/garden/cannabis_garden_bed.tscn` (Bed `dirt_patch_long_horizontal`@0.5 + 3 slots). Ripe stage = `flowering_purple`. **Bed `z_index=0`** (z=−1 sinks below terrain → invisible). **VERIFIED in-engine (fresh load):** grow `stage_1→flowering_purple` per slot over 3 waterings, water consumed, harvest emits → rack receives (`_state 0→1`); slots independent; group=4. **Process incident:** editor autosaved `world.tscn` mid-session and baked duplicate children (`Bed2`/`Slot4-6`) — caught via group count, cleaned on disk; embedded-play PackedScene cache needed `CACHE_MODE_REPLACE` to drop a stale `z=-1`. Plant 0.3 / bed 0.5 scale is first-pass (plants sit slightly high). **Walls rigged (ADR-112, 2026-05-31):** 2 new wall PNGs split/added (`wall_broken_end_long/short.png`, `wall_mid_collapsed.png`). All 9 wall templates at `res://scenes/structures/walls/` restructured `Sprite2D`→`StaticBody2D` root (origin at base, tree convention): per-wall uniform scale normalizes the wall's **tallest part to 24px** (per user 2026-05-31; was 11.25px — NOT yet re-tested in-engine at 24px), base `RectangleShape2D` collider (default layer 1), **`y_sort_offset=7`** (offset 0 could never occlude a low wall — player sort point is 7px below feet, so transition needs to sit at the base). **Verified in running engine:** Erik sorts behind a placed wall with head over the top, and `move_and_collide` confirmed the collider blocks him (test fixture removed after). See VERIFICATION HONESTY RULE. **Hotbar rework (ADR-111, 2026-05-31):** new `res://assets/ui/hotbar17.png` (transparent-hole art) replaces deleted `hotbar_7slot.png`; 7 slots now **absolutely anchored to measured PNG hole rects** (`HOLE_X` const in `hud.gd`) — the old `offset_left=91` misalignment is gone; dark backing panel behind the frame fills the holes (empty slots no longer see-through); **SPC/T prompt overlay system fully removed** (vars, builders, 2 funcs, 4 call sites in world.gd/plant.gd). Playtested ✅. `temp/1x7_hotbar'` staging folder deleted. **Deferred:** bar still stretches 200×36→320×24 so holes are slightly short (icon-scaling polish). **Asset cleanup (2026-05-30):** removed 70 unreferenced PNGs + sidecars — `_archived/{tilemaplayer_icon,town-grass-tile}.png` + 7 erik `_west/` frame dirs (flip_h supersedes left-facing; erik_sprites.tres has 0 west refs). 351 other unreferenced PNGs intentionally KEPT as staged Roadmap content; `willow_idle.png` kept (still referenced). Wall assets: now 9 rigged templates at `res://scenes/structures/walls/` (ADR-112 superseded ADR-110's 0.8-scale Sprite2D format). Walls placed in `World/world.tscn` (10 instances: 8 unique + 2 duplicates). **OPEN:** Orange-fruited mature bush animation misgenerated — needs PixelLab regen. Digging north dir missing. New animations (crafting, digging, eating, jumping, push, trade_item, chop_axe) are in .tres but have no gameplay mechanic yet. **Walk-anim fix (2026-05-31):** `player.gd` `is_moving` now derives from actual post-collision movement (`get_real_velocity()`) instead of intended velocity — walk animation no longer cycles in place when blocked by a wall/collider; facing still updates from input intent. Verified live (blocked→idle, free move→walk). **Static-idle revert (2026-06-01):** user reported the player still "walking while stationary" — investigation confirmed LIVE that `is_moving` is correctly `false` when stopped (the logic fix above holds); the culprit was the *animated* idle (`idle_animated_*`, ADR-109) whose body-sway reads as shuffling at pixel scale. `player_animation.gd:34` default idle reverted `"idle_animated_" + dir` → `"idle_" + dir` (static single-frame). Player now stands motionless when stopped (`idle_side` = 1 frame). Verified live + screenshot. Animated-idle anims remain in erik_sprites.tres, unused (reversible). **Grey-hoodie house swap (2026-06-01):** `HouseTwostoryTeal` node exterior art replaced with new `temp/GreyHoodie2ndHouse.gif` → converted to `assets/structures/houses/house_grey_hoodie_002.png` (768×768, 3×3 grid, 9 frames) + `resources/structures/house_grey_hoodie_002_frames.tres` (idle, 6fps, loop). world.tscn `sprite_frames` repointed (ext id 55_h4nnu reused). Old WIP `house_grey_hoodie_door.png`/`.import` + `_door_frames.tres` DELETED. Verified live: house renders, idle anim plays. `house_grey_teal_frames.tres` now unreferenced (left on disk).
...
2. **Crop / farming system** — Cannabis garden bed DONE (ADR-113): reusable `cannabis_garden_bed.tscn` (3 water-gated slots → drying rack), `plant.gd` parameterized + `garden_plants` group wiring proven. Next: (a) more bed instances / placement; (b) **timed** CropGrowth (real-time growth) using the corn/berry animation sets in `res://assets/nature/crops/` (corn 17f/16f, berry 17f) — distinct from the water-gated mechanic; (c) hoe → tillable soil. Plant 0.3/bed 0.5 scale is first-pass. Orange bush blocked (PixelLab regen).
...
6. **Wall placement + collision** — 9 rigged templates at `res://scenes/structures/walls/` (ADR-112: StaticBody2D root, base collider, `y_sort_offset=7`, tallest part 24px). The walls currently in `world.tscn` are bare dragged Sprite2D (no collision). Next: replace those with template instances so the player is blocked + y-sorts. (Note: per ADR-113, the cannabis bed uses the same group/instance pattern — same workflow.)
...
### Session end — 2026-06-02 (cannabis garden bed + basket-of-apples swap)
- **Game state:** Runnable. Pre-flight ✅ (no `script = null`; only known world_drop_item.gd unused-var warning + transient `gdscript://` MCP-script warnings; output log clean).
- **What changed this session:**
  - **Cannabis garden bed (ADR-113)** — 3-slot water-gated planting bed placed in `world.tscn` at (264,129), replacing decorative `CannabisPlanterTypeA`. Reuses `plant.gd` (`@export plant_stages` + group `garden_plants`), group-loop wiring in `world.gd`, dirt-stripped cannabis sprites (`assets/nature/plants/cannabis/nodirt/*_nodirt.png`) → `resources/plants/cannabis_garden_frames.tres`. New scenes `scenes/interactables/garden/cannabis_plant.tscn` + `scenes/structures/garden/cannabis_garden_bed.tscn`. Ripe = `flowering_purple`. VERIFIED in-engine: grow→harvest→drying-rack loop, 3 independent slots, fresh-load render. See ADR-113 for the editor-autosave/duplicate-children incident + the `CACHE_MODE_REPLACE` cache fix.
  - **stone_pile.png → basket of apples** — user-approved art swap. Overwrote `assets/props/items/stone_pile.png` in place (now 32×32 basket, was 48×48 rocks; same path/UID keeps all refs working incl. the stump-shrine exchange table). Kept native 32×32 so the world drop renders ~16px (≈57% of the 28px player) — satisfies the "≥20% smaller than player" requirement (48×48 would have rendered 24px = only 14% smaller). VERIFIED in-engine: hotbar icon centers via STRETCH_KEEP_ASPECT_CENTERED; dropped basket smaller than player. Staging folder `temp/Basket of apples/` left in place.
- **Open issues:**
  - **Garden bed scale** — plant 0.3 / bed 0.5 is first-pass; plants sit slightly high on the bed (cosmetic nudge available).
  - **Duplicate cannabis/garden PNGs** — 4 byte-identical copies of the wide bed (`Garden Patch`=`planter_type_b`=`dirt_patch_long_horizontal`=`dirt_patch_long_seeded`), plus `stage_3`=`mid_c`=`planter_type_c`, `silver`=`silver_b`, `compact`=`dark_outline`. Left on disk pending cleanup decision (Safety Rule — no unprompted deletes).
  ...
  - **Walls in world.tscn** are bare dragged Sprite2D (no collision), NOT the rigged StaticBody2D templates at `res://scenes/structures/walls/` — re-place with templates if wall collision/occlusion is wanted (ADR-112).
