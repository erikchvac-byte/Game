...
## Where We Are
- **Last completed:** Proximity-based NPC trade + post-trade home cycle. GreyHoodie stops when player within **36px**, faces player (idle_east/west/south), T prompt appears. Press T → bud removed, gem added, NPC pauses 1.5s. After pause NPC walks to its own door, plays `walk_north` 1s, goes invisible. Tracks one full DayNightCycle (`_t` accumulated ≥ 1.0), reappears at NPC door, resets state, resumes patrol to player door. **E = environmental only**. **T = NPC trade only**. ADR-041.
- **Next up:** Teal house collision refinement (door gap + side walls); cave entrance rigging; roof overlay for teal house in Overhead; dedup `shop_apothecary_alt.png`; dedicated gem sprite to replace WaterGem.png placeholder.
