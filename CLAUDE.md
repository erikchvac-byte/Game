...
## Where We Are
- **Last completed:** Three interaction improvements (ADR-058/059/060, 2026-05-17): (1) Hotbar gold border now tracks selected slot unconditionally — `_on_hud_slot_selected` early-return removed, `set_equipped_slot(index)` always called. (2) NPC right-click nav-to-trade — left-click near NPC (within 36px) walks player to trade range then auto-fires trade; NPC is checked first (highest priority) in `_on_right_click`, arrival branch in `_update_mouse_navigation` tracks NPC movement. (3) Trade bug fixes — `gem_ruby.png` replaces `Date-time-Coin.png` (unreadable at hotbar size); `is_interactable()` guard added to T-key and nav arrival paths to block the one-frame double-trigger that overwrote the "Traded: +1 Gem" toast with "No product available". Previously: player chop + trade animations (ADR-057, 2026-05-16).
...
- **Hotbar behavioral fix.** Gold border now tracks the selected slot at all times (empty, non-tool, tool). Root cause was an early return in `_on_hud_slot_selected` that skipped `set_equipped_slot()` when `equipped_tool` didn't change — fixed by always calling it with the current `index`.
- **NPC click-to-trade live.** Left-click near the NPC (within 36px) walks the player to trade range and auto-fires the trade. NPC is highest-priority target in `_on_right_click`. Works from any distance; nav tracks NPC patrol movement each frame. Interaction priority: NPC > tree > terrain.
- **Trade double-trigger fixed.** `is_interactable()` guard on both T-key (`_input`) and nav arrival (`_update_mouse_navigation`) prevents "No product available" from overwriting "Traded: +1 Gem" within the one-frame `_npc_trade_active` lag window.
- **Gem icon.** `gem_ruby.png` replaces `Date-time-Coin.png` as trade reward — legible red gem sphere at hotbar size. `Date-time-Coin.png` was a large UI sprite sheet that rendered as garbage at 24×14px.
