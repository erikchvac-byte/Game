...
## Where We Are
- **Current state (2026-06-02):** Runnable, pre-flight ✅, output log clean (1 known unused-var warning in world_drop_item.gd — not blocking). All systems ADR-105–115 live. Full per-feature history lives in `ADR.md` — this section stays lean (newest decisions only).
...
2. **Crop / farming system** — Cannabis garden DONE (ADR-113 mechanic + ADR-114 depth fix): `cannabis_garden_bed.tscn` is now **dirt-only**; plants are **separate `cannabis_plant.tscn` instances placed directly in World** (water-gated grow → drying rack, `garden_plants` group wiring). **To place a new garden: drop a bed (dirt) scene + N plant instances; control depth with each node's `position.y` (NEVER `y_sort_offset` — inert in 4.6.2).** Next: (a) more garden placements; (b) **timed** CropGrowth (real-time) using corn/berry sets in `res://assets/nature/crops/` (corn 17f/16f, berry 17f) — distinct from the water-gated mechanic; (c) hoe → tillable soil. Plant 0.3/bed 0.5 scale first-pass. Orange bush blocked (PixelLab regen).
...
### Session end — 2026-06-02 (garden depth-sort fix, ADR-114)
- **Game state:** Runnable. Pre-flight ✅ (no `script = null`; editor errors only known world_drop_item.gd unused-var + transient `gdscript://` MCP-script warnings + reload progress_dialog noise; output log clean).
- **What changed this session:** Garden depth sorting (ADR-114). User reported 3 bugs: player behind the dirt when north of it, a leg-flicker at the dirt's top edge, and plants drawing over the player's torso in the bed. Root cause: nested bed didn't flatten into World's y-sort AND **`y_sort_offset` is inert in 4.6.2** (sorting is purely `position.y` — confirmed live). Fix: un-nested the 3 plants to direct World children; `cannabis_garden_bed.tscn` now dirt-only; depth set by node-origin `position.y` via tree-pattern decouple (move origin = sort line, offset child sprite to hold the visual). Dirt instance (264,90)+sprite y=+32; plant instances y=115, `PurplePlant` y=+9, `Shadow` y=+8.61 (art pixel-unchanged). VERIFIED fresh-frame: legs render over dirt (no slice/flicker); player renders over seedlings when standing in the bed; walk-behind preserved north of the plot.
- **Tooling note (this session):** the embedded-play (`embed_subwindows=true`) MCP **screenshot only refreshes on the first capture right after `play_scene`**; any `execute_game_script` after that freezes the capture on a stale frame. Workflow that works: stage state via the player **spawn position** in `world.tscn` (temp edit) → `reload_project` → `play` → first `get_game_screenshot`. The `y_sort_offset` knob is a dead end — always use `position.y`.
- **Open issues:**
  - **Garden plant/bed scale** — plant 0.3 / bed 0.5 first-pass; plants sit slightly high (cosmetic).
  - **Walk-behind balance** — plant sort line at y=115 favors "player in front in bed"; the walk-behind zone is the grass strip north of the plot. Adjustable (raise/lower plant instance y) if more/less occlusion is wanted.
  - **Duplicate cannabis/garden PNGs** — several byte-identical copies on disk (4× wide bed; `stage_3`=`mid_c`=`planter_type_c`; etc.). Left pending cleanup decision (Safety Rule).
