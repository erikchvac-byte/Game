...
## Where We Are
- **Current state (2026-05-26):** Runnable, pre-flight ✅, output log clean (4 lines). Fay Grove exchange fixed — drop item, step away, return to collect reward. ShT now stays above brick wall (MAP_MAX_Y=445). Door entry works. Nav mesh needs rebake after tree repositioning — player nav around trees may be broken until rebaked. NPC GreyHoodie patrol uses NavigationAgent2D (ADR-100). Player click-nav wired to nav mesh (ADR-098/099). NavRegion baked in world.tscn (ADR-097).
