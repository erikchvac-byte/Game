...
## Where We Are
- **Last completed:** Fix ghost tree image + rescale sprites (2026-05-20, ADR-076). Removed 15 duplicate "2" nodes from all 3 choppable trees in world.tscn (phantom nodes created by MCP add_node on top of instanced-scene children). TreeSprite scale 0.5→0.625 (+25%), StumpSprite scale 0.5→0.125 (−75%). Full chop pipeline now clean — no ghost idle image during fall. Playtested ✅.
