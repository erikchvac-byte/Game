...
## Where We Are
- **Last completed:** NPC trade interaction + keybinding standardization. GreyHoodie NPC walks to player bakery door, waits 10s, player presses **T** to trade 1 bud → 1 gem (WaterGem.png placeholder). T prompt (gold-bordered Panel in EPromptArea) appears on NPC arrival, disappears on departure. **E = environmental only** (well, plant, rack, doors). **T = NPC trade only**. Player starts each session with 1 bud in hotbar. `has_item`/`remove_item` added to `hud.gd` + `inventory.gd`. ADR-041.
- **Next up:** NPC home entry/exit playtest feel; teal house collision refinement (door gap + side walls); cave entrance rigging; roof overlay for teal house in Overhead; dedup `shop_apothecary_alt.png`; dedicated gem sprite to replace WaterGem.png placeholder.
