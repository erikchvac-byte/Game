...
### Session end — 2026-06-02 (seed planting, ADR-115)
- **Game state:** Runnable. Pre-flight ✅ (no `script = null`; editor errors only pre-approved known warnings — `tile_bit_tools` nested-UID dups + editor theme-font warnings; output log clean, no Parse/SCRIPT errors).
- **What changed this session:** Seed planting (ADR-115). Garden bed now **starts empty** — removed the 3 pre-placed `CannabisPlant` instances from `world.tscn` (MCP `delete_node`). Player sows sprouts from a **stacked `seed_packets` hotbar item** (`_grant_starting_items` grants ×3, single slot, count badge "3"). `world.gd._try_plant_seed()` runs first in the interact branch: with a seed selected AND the player within `_BED_PLANT_RADIUS` (40px) of the bed, consumes 1 seed and spawns `cannabis_plant.tscn` as a **direct World child** (ADR-114 un-nest rule) at the nearest of 3 `_GARDEN_SLOTS` (`CannabisPlant1/2/3`), wiring its signals (`interactable_entered/exited`, `plant_harvested→DryingRack`) into the existing growth loop; otherwise returns false so normal interact (water/well/chop) proceeds. Watering/growth (`plant.gd`) untouched. VERIFIED in-engine: seed count 3→2→0, sprouts at correct slots (stage-0 seedling visual), watering advanced stage 0→1 (frame 0→1, visibly taller); node-name fix re-verified after reload.
- **Tooling note:** contrary to the prior session's note, screenshots **did** refresh after `execute_game_script` this session (this was a non-embedded play). Dynamic test workflow that worked: `play` → `execute_game_script` to position player / set `_hud.selected_slot` / call `world._try_plant_seed()` → `get_game_screenshot`. `y_sort_offset` remains a dead end — always use `position.y`.
- **⚠ PENDING — push not completed:** the 2 commits (ADR-115 code + docs) are local on `main`. `git push` is blocked: local `main` tracks `origin/master` and the safety classifier denied a direct default-branch push. To finish: user runs `! git push origin HEAD:master`, OR push to a feature branch + PR, OR add a Bash permission rule.
- **Open issues:**
  - **Garden slots hard-coded to one bed** — `_GARDEN_SLOTS`/`_bed_slots` in `world.gd` assume the single bed at (264,90). A second bed needs slot geometry generalized (per-bed node or computed offsets).
