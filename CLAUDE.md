...
- **Axe/tool pattern:** `player.equipped_tool: String` (empty = nothing equipped). C → `_handle_axe_toggle()` checks `InventoryManager.has_item("axe")`, flips `player.equipped_tool`, then calls `hud.set_equipped_slot(slot_idx)` to light up the axe slot in gold. No axe animation yet.
- **Hotbar indicators:** `hud._slot_style(selected, equipped)` — white border = keyboard cursor, gold border = equipped tool. `hud.set_equipped_slot(index)` is the push API; index=-1 clears. `_on_slot_changed` applies 50% inset for `key == "wood"` (rock3.png placeholder is oversized).
...
- **Tree scene pattern:** `res://World/ChoppableTree/choppable_tree.tscn` — exported vars: `tree_texture`, `stump_texture`, `tree_visual_scale`, `stump_offset`, `stump_visual_scale`, `stump_flip_h`, `chops_required`. Each instance in world.tscn overrides these. New trees = duplicate the scene + drop in world. Signals: `interactable_entered/exited` (SPC prompt), `wood_chopped` (grants wood via world.gd). `can_interact(player)` checks `player.equipped_tool == "axe"` — world.gd shows toast if false.
...
- **Drying rack bug fixed:** `_award_and_reset()` was calling `/root/Inventory` (old autoload, non-existent). Now correctly calls `/root/InventoryManager`. Bud rewards now land in inventory.
