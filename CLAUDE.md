...
## Where We Are
- **Last completed:** Spacebar interact rebind + wood inventory bug fix (ADR-046, 2026-05-15). `interact` action rebound E→Space in project.godot. HUD SPC panel prompt (28px, styled to match T-prompt) replaces key_e.png TextureRect. Bug fix: `_on_wood_chopped()` and `_grant_starting_items()` in world.gd used `/root/Inventory` (non-existent) — corrected to `/root/InventoryManager`. Play-tested: SPC prompt shows near trees, chop → stump, wood 1→2 confirmed. Previously: choppable trees (ADR-045).
- **Interact key:** Space (keycode 32). All interactables (well, plant, drying rack, trees) respond to Space via the `interact` named action. T key = NPC trade only. C key = axe equip toggle.
...
- **Axe/tool pattern:** `player.equipped_tool: String` (empty = nothing equipped). C key → `_handle_axe_toggle()` in world.gd checks `InventoryManager.has_item("axe")` then flips `player.equipped_tool`. No axe animations yet — equip state is data-only.
- **Tree scene pattern:** `res://World/ChoppableTree/choppable_tree.tscn` — exported vars: `tree_texture`, `stump_texture`, `tree_visual_scale`, `stump_offset`, `stump_visual_scale`, `stump_flip_h`, `chops_required`. Each instance in world.tscn overrides these. New trees = duplicate the scene + drop in world. Signals: `interactable_entered/exited` (SPC prompt), `wood_chopped` (grants wood via world.gd).
- **Wood placeholder:** `res://GameAssets/Caves/Rocks/rock3.png` used as wood icon. Key = `"wood"`. Stacks correctly, granted at start + on each chop.
