...
enum State { IDLE, CHOPPING, FALLING, STUMP, GONE }
...
func _ready() -> void:
	...
	_stump_sprite.animation_finished.connect(_on_stump_anim_finished)
...
func _on_tree_anim_finished() -> void:
	if _state == State.CHOPPING:
		...
		_stump_sprite.play("dissolve")
		_trunk_col.disabled = true
		_interact_col.disabled = true
		_state = State.STUMP
		tree_chopped.emit()

func _on_stump_anim_finished() -> void:
	_stump_sprite.visible = false
	_state = State.GONE
